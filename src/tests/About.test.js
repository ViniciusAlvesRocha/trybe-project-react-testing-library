...
test('Vrificando de a gágina About da Pokédex contem dois paragrafos', () => {
  ...
  const expression1 = /a Pokédex, a digital encyclopedia containing all Pokémons/;
  const firstParagraph = getByText(expression1);
  const expression2 = /er Pokémons by type, and see more details for each one of them/;
  const secondParagrapy = getByText(expression2);
  expect(firstParagraph).toBeInTheDocument();
  expect(secondParagrapy).toBeInTheDocument();
});

test(`Verificando se a página contem
o link da imagem de background da página About`, () => {
  const { getByRole } = renderWithRouter(<About />);
